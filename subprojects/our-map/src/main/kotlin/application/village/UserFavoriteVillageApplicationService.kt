...
class UserFavoriteVillageApplicationService(
    ...
    fun register(userId: String, villageId: String) = transactionManager.doInTransaction {
        val user = userRepository.findById(userId)
        val village = villageRepository.findById(villageId)
        userFavoriteVillageService.register(user, village)
    }

    fun unregister(userId: String, villageId: String) = transactionManager.doInTransaction {
        val user = userRepository.findById(userId)
        val village = villageRepository.findById(villageId)
